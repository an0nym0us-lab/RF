...
class CountingBloom{
    ...
    int counting_bloom_add_IP32(size_t &ip);
    int counting_bloom_remove(Slice &key);
    int counting_bloom_check(Slice &key);
    int counting_bloom_check_IP32(size_t &ip);
...
int CountingBloom::counting_bloom_add_IP32(size_t & ip){
    ...
    for(uint8_t j=0; j<nfuncs; j++){         
        // unsigned int bitpos = XXH3_128bits_withSeed(&ip, 32, j+1).low64 % numCounter;

        uint64_t buf[2];
        MurmurHash3_x64_128(&ip, 8, j+1, buf);
        unsigned int bitpos = buf[0] % numCounter;
        // MurmurHash3_x64_128(&ip, sizeof(size_t), 0, &bitpos);
        bitmap_increment_adapt(this->bitmap, bitpos, offset);
    }
    this->header->count++;
    return 0;
...
int CountingBloom::counting_bloom_check_IP32(size_t & ip){


    for(int i=0 ; i<nfuncs; i++){
        // uint32_t bitpos = XXH3_128bits_withSeed(&ip, 32, i+1).low64 % numCounter;

        uint64_t buf[2];
        MurmurHash3_x64_128(&ip, 8, i+1, buf);
        unsigned int bitpos = buf[0] % numCounter;
        // uint32_t bitpos;
        // MurmurHash3_x64_128(&ip, sizeof(size_t), 0, &bitpos);
        bitpos = bitpos % numCounter;
